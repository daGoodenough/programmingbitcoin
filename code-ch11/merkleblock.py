...
class MerkleBlock:
    ...
    def parse(cls, s):
        ...
        total_txes = little_endian_to_int(s.read(4))
        # number of transaction hashes - varint
        total_hashes = read_varint(s)
        # each transaction is 32 bytes, Little-Endian
        tx_hashes = []
        for tx in range(total_hashes):
            tx_hashes.append(s.read(32)[::-1])
        # length of flags field - varint
        flags_len = read_varint(s)
        # read the flags field
        flags = s.read(flags_len)
        # initialize class
        return cls(version, prev_block, merkle_root, timestamp, bits, nonce, total_txes, tx_hashes, flags)
